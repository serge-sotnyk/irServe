...
| **7e** | HTTP compression (`-u`/`--no-compression` un-no-op) | SRV-CLI-012 (P2) | `015-compression` | — | **DONE.** Closed Q-002 via `tools/probe/snapshots/compression-raw.json` (slice 0 — commit `9bd52f0`): threshold = 1024 bytes; negotiation order = brotli > gzip > deflate; MIME set = curated allowlist (`application/json`, `application/javascript`, `application/wasm`, `image/svg+xml`) + regex fallback `^text/|\+(?:json|text|xml)$/i`; skip conditions = HEAD / `Cache-Control: no-transform` / below-threshold / identity-only / all-q-zero; OPTIONS routes through GET pipeline (Stage 7d composition); Range pre-empts compression. Wired `-u` / `--no-compression` CLI flag, added `compression: Option<bool>` to `ServeConfig`, authored `crates/irserve-core/src/compression.rs` (`negotiate` / `is_compressible` / `maybe_apply` / `encode`), and integrated the seam into `build_file_or_304` after `apply_custom_headers` and before Range (slice 2 — commit `d1b9a15`). Added `flate2 = "1"` and `brotli = "7"` to `crates/irserve-core/Cargo.toml`. Probe partitions: `runner.l0.clean` over all 20 anchors of `compression-raw.json` plus `bodyMayDiffer` over the 10 compressed anchors per D-020 #4 (encoder defaults differ at the bit level between Node and Rust). Promoted ORC-058 (`compression-default.json`) and added ORC-191..ORC-210 (`compression-raw.json`) — all dual-target. D-006 transitions from `adapted` (100 % deferred) to implemented; **D-020** records the four surviving divergences from reference (framing — `Content-Length` not chunked; no `mime-db` table port; q-rank within `(0,1)` not honored; compressed body bytes not byte-identical). `cargo test --test oracle` ends at **81 passed, 2 skipped, 0 failed** (was 79 / 3 / 0 at end of Stage 7d). |
