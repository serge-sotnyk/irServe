...
Codex review round 4 added two refinements to the glob fallback. **P1 (leading-dot rejection):** minimatch's default is `dot: false`, where a `*` pattern segment does NOT match a path segment beginning with `.`. globset matches dotfiles, so we needed a post-validation step. The `Option<GlobMatcher>` field was upgraded to a struct `GlobFallback { matcher, pattern_segments, has_doublestar }`; `matches_strict` runs the globset match first, then walks pattern segments alongside path segments and rejects when any `*`-bearing pattern segment aligns with a leading-`.` path segment. Pinned by ORC-098/ORC-099 (`/a/.x/b/y` and `/a/x/b/.y` against `/a/*/b/*` both 404). **P2 (`:name`+`*` fallback):** the round-3 implementation gated `glob_fallback` construction on `!has_path_param(&body)` based on the assumption that minimatch would never match for `:name`-bearing sources (because `:name` would never appear literally in a path). Codex showed that contrived but valid requests can carry literal `:name` segments (URL paths permit `:` unencoded), in which case the reference's minimatch fallback DOES fire and emit a 301. The gate was removed so `glob_fallback` builds for any `*`-bearing source. Pinned by ORC-100 (`/a/:id/x/b/y` against `/a/:id/*/b/*` returns 301 via the fallback).
