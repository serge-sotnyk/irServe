...
Codex review round 3 P1 added a `glob_fallback: Option<GlobMatcher>` field to the `Pattern` matcher for `*`-bearing-no-`:name` sources, mirroring the reference's `sourceMatches` two-stage matching at `index.js:46-67`: try `pathToRegExp` first, fall back to `minimatch` on null. With `path-to-regexp@3.3.0` not recognizing bare `*` as a wildcard token, multi-`*` sources like `/a/*/b/*` produce a regex with literal `\*` characters that real URLs never match — the glob fallback (a `globset::Glob` over the original `*`-bearing source) recovers minimatch's single-segment-per-`*` behavior. Pinned by `redirects-glob-source.json`: positive case `/a/x/b/y` matches (ORC-095, anchor `multi_star_matches_single_trailing`), negative cases stay 404 (ORC-091 multi-segment trailing, ORC-096 no trailing segment, ORC-097 extra middle segment). The fallback trims a single trailing `/` from the path before matching (mirrors `path.posix.resolve` in `index.js:41`) so that globset's zero-or-more `*` semantics line up with minimatch's one-or-more.
