...
Status: adapted
Reason: Change `004-route-normalization` (Stage 6b) wires phases 3 and 5 of the 13-phase dispatcher: silent multi-slash collapse before any routing decision (SRV-ROUT-005, Q-006 closed) and `trailingSlash` 301 add/strip on the normalized path (SRV-ROUT-003, SRV-ROUT-004). Two new probes (`trailingslash-add`, `trailingslash-strip`) isolate the trailingSlash behavior from `cleanUrls` (which defaults to on in `vercel/serve` but is explicitly disabled in the new fixtures) so the redirect contract can be validated against `target=irserve` without depending on phase 4. The compose probes (`prec-cleanurls-trailing`, `prec-cleanurls-trailing-false`) and the cleanUrls-dependent anchors of `multislash-collapse` (`double_slash_segment`, `internal_double_slash`) stay reference-only — they are skipped or marked `divergent` in `runner.l0` until Stage 6c lands phase 4. SRV-ROUT-003 and SRV-ROUT-004 oracle lists grow by ORC-068/069 and ORC-070/071 respectively. SRV-ROUT-006 (precedence) stays deferred because phases 4, 6, 7, 8 are not yet implemented.
Impact: IrServe now silently collapses runs of `/` in the request path (the collapse itself never emits a 301), and emits a 301 to `path + "/"` (or to the slash-stripped form) when `trailingSlash` is `true` (`false`) and the path is eligible. Dotfile and extension exemptions on the add branch match the reference's `path.parse(p).{name,ext}` shape verbatim; the strip branch ignores those exemptions, also matching the reference. The redirect's `Location` header is set to the target verbatim (no percent-encoding); axum/hyper's auto-emitted `content-length: 0` on the empty 301 body is masked via `runner.l0.contentLengthMayDiffer` since the reference's Node http response omits it. The remaining routing SRVs (SRV-ROUT-001/002 cleanUrls, SRV-ROUT-006 precedence) stay deferred per D-008.
