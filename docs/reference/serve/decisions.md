...
Status: adapted
Reason: Change `006-configured-redirects` (Stage 6d) wires phase 6 of the 13-phase dispatcher. Source patterns route through three compiled matchers in `crates/irserve-core/src/redirects.rs`: `Literal` (no glob meta, no `:name`; trailing-slash flexion mirrors `pathToRegExp("/old", []) = ^/old/?$`), `Glob` (compiled via `globset::GlobBuilder` with `literal_separator(true)` so a single `*` does not cross `/`; `!`-prefix negation through the same XOR pattern as cleanUrls), and `Pattern` (sources with `:name` segments compiled into a `regex::Regex` with named capture groups, mirroring `serve-handler/src/index.js:46-49`'s `slashed.replace('*', '(.*)')` + `pathToRegExp` first-pass). The destination is pre-parsed into a `DestTemplate` of literal + `:name` fragments; per-value `encodeURIComponent` is applied at render time, mirroring `pathToRegExp.compile`'s per-prop encoding (`index.js:81-87`). The surrounding `encodeURI` over the full target lives in `dispatch::encode_uri_target` (already existed since 6b). `compute_configured_redirects` walks the rules in order and returns the first match's `(target, status_code)`; status defaults to 301 and uses the rule's `type` field when present, mirroring `index.js:179`. Out-of-range or otherwise invalid `type` values fall back to 301 (matches "accept any 3xx; range-checking is not specified" per `openspec/specs/redirects/spec.md` SRV-RDIR-002 Note). `compile_rules` returns `(Vec<RedirectRuleCompiled>, Vec<InvalidRedirect>)` and the bin layer emits one stderr warning per skipped rule, mirroring the cleanUrls treatment from 6c. Workspace pin `regex = "1"` (latest stable 1.12, verified via context7 on 2026-05-09).

`!`-prefix combined with a `:name` segment is rejected at compile time via a new `CompileError::NegatedParam` variant. Path-to-regexp has no negation flag, and the reference's minimatch fallback for that combination treats `:name` as literal characters that no real request path will match — the rule never fires there either. We surface the rejection as a warning rather than silently producing a never-matching rule.

Q-007 (relative / scheme-relative / absolute Location forms) is closed by snapshot `tools/probe/snapshots/redirects-destination-forms.json` (4 anchors: ORC-079 `absolute_https`, ORC-080 `scheme_relative`, ORC-081 `relative_no_leading_slash`, ORC-082 `absolute_path_baseline`). The reference's destination-side `slasher` is `glob-slash`'s `path.posix.normalize` plus a leading-slash guarantee (`index.js:80`, `protocol ? destination : slasher(destination)`). The Q-007 surprise is that `path.posix.normalize` collapses consecutive slashes in the destination — so a scheme-relative `//example.com/x` is normalized to `/example.com/x` and emitted as a same-origin redirect, not as a true scheme-relative URL. Absolute URLs (`https://example.com/x`) skip normalization and pass through verbatim; relative paths (`foo/bar`) get a leading slash; absolute paths (`/foo/bar`) pass through unchanged. IrServe mirrors all four exactly via `redirects::normalize_destination` (which uses `crate::normalize::collapse_slashes` for the consecutive-slash collapse — fuller `path.posix.normalize` semantics like `.`/`..` resolution are not implemented because real-world redirect destinations don't carry those segments; if a divergence surfaces later, escalate to a Q-NNN). SRV-RDIR-003 is promoted from `accepted` to `verified`. Q-012 (extglob) limitation is inherited unchanged: redirects glob sources use the same `globset` matcher as cleanUrls and so do not honor `+(...)`, `@(...)`, `?(...)`, `*(...)`, `!(...)` extglob constructs.
