...
- Existing test: absent.
- Probe: `tools/probe/cases/compression-default.json` (single-anchor `Vary: Accept-Encoding` check, dual-target after Stage 7e slice 2); `tools/probe/cases/compression-raw.json` (22 raw-socket anchors — 20 authored in Stage 7e slice 0, plus 2 added in Codex round 3 P2 for the identity / non-identity `Content-Encoding` passthrough dichotomy — pinning the threshold / negotiation / MIME filter / skip conditions / Range pre-emption / user-`Content-Encoding` passthrough — closes Q-002).
- Oracle test: ORC-058 (`compression-default.json#with_accept_encoding`, dual-target post-7e), ORC-191..ORC-212 (`compression-raw.json` 22 anchors, all promoted dual-target via the slice-2 `runner.l0.clean` partition with `bodyMayDiffer` on the 11 compressed anchors per D-020 #4).
