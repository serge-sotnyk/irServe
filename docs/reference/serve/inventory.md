...
- Existing test: absent.
- Probe: `tools/probe/cases/compression-default.json` (single-anchor `Vary: Accept-Encoding` check, dual-target after Stage 7e slice 2); `tools/probe/cases/compression-raw.json` (23 raw-socket anchors — 20 authored in Stage 7e slice 0, plus 2 added in Codex round 3 P2 for the identity / non-identity `Content-Encoding` passthrough dichotomy, plus 1 added in Codex round 4 P2 for the above-threshold-range encode case — pinning the threshold / negotiation / MIME filter / skip conditions / Range threshold / user-`Content-Encoding` passthrough — closes Q-002).
- Oracle test: ORC-058 (`compression-default.json#with_accept_encoding`, dual-target post-7e), ORC-191..ORC-213 (`compression-raw.json` 23 anchors, all promoted dual-target via the slice-2 `runner.l0.clean` partition with `bodyMayDiffer` on the 12 compressed anchors per D-020 #4).
...
- Probe: `tools/probe/cases/redirects-types.json` (path-segment redirect with default 301), `tools/probe/cases/redirects-glob-source.json` (`*`-source cross-segment, `**`-collapse, multi-`*` minimatch-fallback positive/negative + leading-`.` rejection), `tools/probe/cases/redirects-source-slasher.json` (source-side `path.posix.normalize` parity), `tools/probe/cases/redirects-negation-source.json` (`!`-prefix + `:name` falls through to minimatch), `tools/probe/cases/redirects-pattern-with-multistar-fallback.json` (`:name`+multi-`*` falls to minimatch on literal `:name` paths), `tools/probe/cases/redirects-glob-edges.json` (Glob `[.]y` `dot: false`, brace expansion before dot rule, `:name`+`?` minimatch fallback), `tools/probe/cases/redirects-negation-bracket.json` (negation flips bracket-pattern dot rejection), `tools/probe/cases/redirects-resolve-and-doublestar.json` (path.posix.resolve trim parity, `**` after `:name` admits zero, `**` middle admits zero), `tools/probe/cases/redirects-negation-doublestar.json` (negation+`**`-end strict matcher), `tools/probe/cases/redirects-resolve-and-escape.json` (full path.posix.resolve `.`/`..` plus `\.` escape parity for Literal and Wildcard with `\.`), `tools/probe/cases/redirects-backslash-meta.json` (`\*`/`\?`/`\[`/`\{` keep glob meta per minimatch parity), `tools/probe/cases/redirects-edge-corners.json` (`\**` globstar via post-de-escape `**` check, brace per-alt dot rule, Literal fallback on globset error), `tools/probe/cases/redirects-backslash-paths.json` (trailing-`\` and inner-`\X` source patterns vs requests with `%5C` literal backslash — Literal `source_ptr` + `source_mm` dual-form, Pattern glob_fallback gating), `tools/probe/cases/redirects-case-insensitivity.json` (path-to-regexp default `i` flag — Literal and Pattern matchers are case-insensitive; Glob and glob_fallback remain case-sensitive per minimatch default), `tools/probe/cases/redirects-latin1-case.json` (Latin-1 case-folding via Unicode-aware `to_lowercase`; covers Ä/ä, É/é, Ö/ö, Ü/ü).
- Oracle test: ORC-030, ORC-084..ORC-147 (snapshots in tools/probe/snapshots/).
...
- `\*\*` (escaped consecutive stars) parses as two single-segment globs in minimatch but as DoubleStar/globstar in IrServe (de-escape happens before the `**` classifier check) — documented in D-012, pinned by ORC-128.
...
Reference source:
- README: absent.
- serve-handler source: `src/index.js:717-734` and `src/index.js:749-752` (range plumbing). Range parser: `third_party/serve/node_modules/.pnpm/range-parser@1.2.0/node_modules/range-parser/index.js:42-71`.
- Existing test: `range request`, `range request without size`, `range request not satisfiable` in `test/integration.test.js`.
- Probe: `tools/probe/cases/range-request.json` — 21 anchors. The `runner.l0.clean` partition has 18 anchors (single-range surface, dual-target byte-identical); `runner.l0.divergent` has 3 multi-range anchors (`range_multi_first_invalid`, `range_multi_first_unsatisfiable`, `range_multi_first_alpha_only`) recording the D-019 divergence reference-only; `bodyMayDiffer: [range_on_missing_file]` covers the synthetic-HTML-404 D-002 carve-out. Stage 7c grew the case in three batches: slice 0 added 5 edge-case anchors (`suffix_last_3`, `single_byte`, `clip_to_end`, `range_on_missing_file`, `range_with_inm_match`), Codex round 1 added 3 (`range_on_empty_file`, `range_416_user_content_range_override`, `range_trailing_garbage_in_end`), round 2 added 7 parser-corner anchors (`range_plus_sign_in_start`, `range_plus_sign_in_end`, `range_plus_sign_in_suffix`, `range_overflow_end_clipped`, `range_double_hyphen_open_end`, `range_alpha_start_is_suffix`, `range_suffix_larger_than_total_is_416`), round 3 added 3 multi-range divergent anchors.
- Oracle test: ORC-044, ORC-045, ORC-046, ORC-173..ORC-187 (single-range, dual-target after Stage 7c slice 2 + rounds 1/2), ORC-188..ORC-190 (multi-range, reference-only per D-019). Snapshots in `tools/probe/snapshots/range-request.json`.

Requirement (draft):
A request with a valid single-range `Range: bytes=...` header for a file of known size returns status 206 with `Content-Range: bytes <start>-<end>/<total>` and `Content-Length: <end-start+1>` (the partial body is `bytes[start..=end]`, inclusive end). A strictly-out-of-range or otherwise unsatisfiable value returns 416 with `Content-Range: bytes */<total>` and the full file as the response body (mirrors reference fall-through; RFC 7233 §4.4). The single-range parser surface mirrors `range-parser`'s `parseInt`-style leniency (sign prefix, trailing garbage, u64-overflow saturation), `split('-')` semantics on multi-hyphen forms, and `isNaN(start)` suffix-repurpose — see Stage 7c Codex round 1 and round 2 commits and the change package's `specs/http-cache/spec.md` for the per-corner mapping. A `Range` header bearing a comma-separated multi-range list is **partially adapted** per **D-019**: irserve consults only the first comma-segment and returns 416 if that segment alone is invalid, regardless of whether later segments would be valid; reference iterates all segments and takes the first VALID range. The single-range surface is the contract; the multi-range divergence is documented in D-019 and pinned by the three divergent probe anchors.

Compatibility notes:
- **Multi-range comma list is first-segment-only (D-019, parity scope declared per anti-hallucination rule #9 after three Codex review rounds on the parser).** Reference iterates ALL comma-segments at `range-parser/index.js:42-71` (skipping invalid via the `continue` at `:63`) and `serve-handler` takes `range[0]` from the resulting list of VALID ranges at `index.js:724`. irserve takes only the first comma-segment unconditionally. Pinned by `range-request.json#range_multi_first_invalid` / `#range_multi_first_unsatisfiable` / `#range_multi_first_alpha_only` (`runner.l0.divergent`, reference-only); irserve-side outcome unit-tested as `range::tests::parse_multi_first_*_irserve_side`. Closing the divergence is a contained ~10 LOC change; would flip D-019 from `adapted` to `mirrored`.
- `If-Range` is read off the request and ignored (reference TODO at `index.js:719`).
...
- 416 user `Content-Range` override wins over the default `bytes */<total>` (Codex round 1 P2 fix); 206 range-emitted `Content-Range`/`Content-Length` wins over user values. The asymmetry mirrors reference's setHeader-before-getHeaders (416) vs post-getHeaders injection (206) ordering. Pinned by `range_416_user_content_range_override` (ORC-179) for the 416 path.
- Empty-file Range request returns 200 + empty body + no `Content-Range` — mirrors reference's `if (request.headers.range && stats.size)` guard. Pinned by `range_on_empty_file` (ORC-178).
