...
How to verify:
- Probe `tools/probe/cases/redirects-destination-forms.json` (7 anchors: `absolute_https`, `scheme_relative`, `relative_no_leading_slash`, `absolute_path_baseline`, `dotdot_resolved` (mid-path `..`), `dotdot_leading_resolved` (leading `..`), `empty_destination_root`).
Resolution: closed by snapshot `tools/probe/snapshots/redirects-destination-forms.json`. ORC-079, ORC-080, ORC-081, ORC-082, ORC-086, ORC-087, ORC-088. The reference's destination-side `slasher` (`serve-handler/src/index.js:80`, `protocol ? destination : slasher(destination)`) is `path.posix.normalize(path.posix.join('/', value))` per `glob-slash.js:6`. Absolute URLs (those whose `url.parse(...).protocol` is truthy) skip normalization and pass through verbatim. Everything else goes through `slasher`. Surprises: (1) `path.posix.normalize` collapses consecutive slashes, so a scheme-relative `//example.com/x` becomes `/example.com/x` (same-origin redirect, not a true scheme-relative URL); (2) `path.posix.normalize` resolves `.`/`..` segments, so `a/../b` becomes `/b` and `../b` joins-then-drops to `/b`; (3) an empty destination joins to `/` and normalizes to `/`. Relative paths without `..` (`foo/bar`) get a leading `/`; absolute paths (`/foo/bar`) pass through unchanged. IrServe mirrors via `crates/irserve-core/src/redirects.rs::normalize_destination` + `slasher_join_normalize` + `path_posix_normalize`. SRV-RDIR-003 is promoted from `accepted` to `verified`.
...
Suspected behavior: `serve-handler` calls `minimatch` via `sourceMatches` (`serve-handler/src/index.js:38-67`), which honors **extglob** constructs by default — Bash-style extended glob patterns where `+(a|b)` matches one or more occurrences of `a|b`, `@(a|b)` matches exactly one, `?(a|b)` matches zero or one, `*(a|b)` matches zero or more, and `!(a|b)` matches anything except. The `sourceMatches` helper is shared between `applicable` (used by `cleanUrls`, `serve-handler/src/index.js:265`) and `toTarget` (used by `redirects`/`rewrites`, `serve-handler/src/index.js:70`), so any extglob support visible on one path is structurally visible on the other. The pinned reference's only dedicated extglob test exercises the redirects branch (`serve-handler/test/integration.test.js:449`, `redirects: ["face/+(mask1|mask2)/ideal"]`); the cleanUrls array test at `integration.test.js:705` uses a plain `/directory**` glob, so cleanUrls extglob is structurally implied (same `sourceMatches` call) but not directly tested upstream. Probe-confirmed for the cleanUrls path via Codex review round 3 (`cleanUrls: ["/public/+(page|other).html"]`, `GET /public/page.html` → reference `301 Location: /public/page`); recorded as `tools/probe/snapshots/cleanurls-extglob.json` in this repo.
How to verify:
