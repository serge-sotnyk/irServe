...
| ORC-122 | redirects          | `GET /g2/.x` with redirect `/g2/\.* → /escape-glob-hit`           | SRV-RDIR-001 (Wildcard with `\.` admits dotfile) | `cases/redirects-resolve-and-escape.json#escape_glob_with_dot_admits_dotfile` | must-match: status=301, `location: /escape-glob-hit` (the Wildcard segment `\.*` has effective-leading-dot — the `\` escapes a literal `.`, so the dot rule admits `.x`. Globset with `backslash_escape(true)` compiles `\.*` to match `.X` literally). Closes Codex round 8 P2 (Wildcard escape). | verified |
