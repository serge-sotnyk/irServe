...
| ORC-091 | redirects          | `GET /a/x/b/y/z` with redirect `/a/*/b/* → /multi-hit`            | SRV-RDIR-001 (multi-`*` no over-match)           | `cases/redirects-glob-source.json#multi_star_no_overmatch`       | must-match: status=404 (only the FIRST `*` becomes `(.*)`; the second `*` becomes a literal that real URLs don't carry, so the rule does NOT fire on `/a/x/b/y/z`). Body may differ between reference and irserve per D-003. Closes Codex round 2 P1 (multi-`*` over-match). | verified |
