...
| **6d** | Configured redirects (done — `006-configured-redirects`) | SRV-RDIR-001, SRV-RDIR-002, SRV-RDIR-003 | `006-configured-redirects` | 6a, 6c (precedence per SRV-ROUT-006) | Closes Q-007 (relative / scheme-relative / absolute Location forms — surprise: scheme-relative `//host/x` collapses to `/host/x` because `glob-slash.slasher` runs `path.posix.normalize` first, see D-012). Wires phase 6. Closes the trailingSlash↔redirects compose corner of SRV-ROUT-006 (ORC-083); cleanUrls↔redirects already covered by ORC-033. Inherits Q-012's no-extglob limitation for glob source patterns. |
| **6e** | Configured rewrites + `--single` SPA (done — `007-configured-rewrites`) | SRV-RWRT-001, SRV-CLI-008 | `007-configured-rewrites` | 6a, 6d | Wires phase 7 via shared `path_pattern.rs` matcher (extracted from redirects in slice 1). Recursive chaining mirrors `applyRewrites` at `serve-handler/src/index.js:91-117` with an irserve-only depth cap of 64 (D-014). `--single` injects `{source: "**", destination: "/index.html"}` at config-load time per `main.ts:78-90`. Closes the redirects↔rewrites compose corner of SRV-ROUT-006 (D-013). |
| **6f** | Custom error pages, full L2 security, custom response headers (done — `008-error-pages-and-security`) | SRV-FILE-003, SRV-SEC-001 (full surface), SRV-SEC-002, SRV-HDR-001, SRV-HDR-002 | `008-error-pages-and-security` | 6a (custom headers via config) | Generalizes phase 13 (`error.rs`'s `error_response(status, headers, root)` with `<status>.html` lookup at served root); tightens phase 1 (strict `%xx` decode → 400) and phase 10 (lexical `..` containment → 400, defense-in-depth `EscapedRoot` arm); adds post-dispatch `apply_custom_headers` pass that mirrors `getHeaders` (accumulate, case-insensitive override, `value: null` prune; 3xx redirects skipped). Methodological signals captured in D-015: `traversal-raw-encoded.json` snapshot resync, 3xx-skip empirical finding, 400-no-headers minimatch nuance, and `@zeit/schemas` rejection of `value: null` (SRV-HDR-002 verified by unit tests rather than oracle probe). |
| **6g** | Directory listing (done — `009-directory-listing`) | SRV-DLST-001, SRV-DLST-002, SRV-DLST-003 | `009-directory-listing` | 6a, 6c (URL trailing slash interaction), 6f (404 fallback when listing disabled) | Wires phase 11 (directory branch). HTML / JSON content negotiation; `unlisted` defaults (`.DS_Store`, `.git`) plus user globs; `renderSingle` short-circuit (count check is pre-`unlisted`-filter, mirroring reference). Apply D-007 sanitization with `"."` / `"sub"` / `"sub/deep"` shape (no leading/trailing slash). Listing 200 responses bypass `apply_custom_headers` (mirrors reference's bypass of `getHeaders` at `serve-handler/src/index.js:644-672`); `renderSingle` does go through custom headers. No new D-NNN authored — divergences flow from existing D-003 (HTML markup) and D-007 (JSON sanitization). |
| **6h** | CLI fill-in (done — `010-cli-fill-in`) | SRV-CLI-003 (`tcp://`), SRV-CLI-006 (`-p` alias), SRV-CLI-010 (`--cors` — L1 ACAO only), SRV-CLI-014 (`--debug`), SRV-CLI-015 (`--no-request-logging`), SRV-CLI-016 (`--no-port-switching`) | `010-cli-fill-in` | — | Closes Q-001 (`tcp://` host/port defaults). SRV-CLI-009 (`--config`) was closed in 6a as a side-effect of the loader. New D-016 documents the upstream `--no-port-switching` no-op (vercel/serve#751); irserve enforces the documented contract. Several flags (`--debug`, log format) are partial no-ops under D-002. The full L3 CORS response surface stays deferred to Stage 7. |
