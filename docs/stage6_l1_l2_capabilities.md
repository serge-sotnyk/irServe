...
| **6c** | cleanUrls | SRV-ROUT-001, SRV-ROUT-002 | `005-clean-urls` | 6a, 6b | Wires phases 4 + 8. Flips `_smoke#index_html_redirect` and `mime-defaults#html` from L1-divergent to L0-clean. Also unblocks the `default-port-l0` no-flag-no-env probe (which currently sidesteps the cleanUrls divergence). |
...
| **6h** | CLI fill-in | SRV-CLI-003 (`tcp://`), SRV-CLI-006 (`-p` alias), SRV-CLI-009 (`--config`), SRV-CLI-010 (`--cors` — L1 ACAO only), SRV-CLI-014 (`--debug`), SRV-CLI-015 (`--no-request-logging`), SRV-CLI-016 (`--no-port-switching`) | `010-cli-fill-in` | 6a (for `--config`) | Closes Q-001 (`tcp://` host/port defaults). Several flags are no-ops under D-002. The full L3 CORS response surface stays deferred to Stage 7. |
