...
9. **Stop after 3+ consecutive review rounds on the same subsystem.** If reviewer findings keep landing on one matcher, one helper, or one aspect for three rounds running, do not start the fourth fix immediately. Pause and: (a) declare a compatibility level for that subsystem (L1 functional parity vs L2 bit-for-bit), (b) enumerate known-but-unfixed divergences explicitly in the relevant `D-NNN` decision, (c) ask the user whether to continue iterating or freeze. The default presumption is that the cluster of remaining cases is rare and belongs in known divergences, not in another fix round.
