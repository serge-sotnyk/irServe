...
Path-traversal denial and URL decoding guarantees that prevent escape
from the served root. This capability fixes the wire-level behavior
(literal `..`, percent-encoded `%2e%2e`, leading `//`, malformed
`%`-escape) and the single-decode invariant.
