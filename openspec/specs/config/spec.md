...
`serve.json` configuration loading and the schema map that drives
behavior of routing, listing, and content negotiation. Field-by-field
semantics live in the per-area capability specs; this capability fixes
the lookup order, error handling, and the inventory of accepted fields.
