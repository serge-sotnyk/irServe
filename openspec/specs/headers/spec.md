...
Custom response headers driven by `serve.json`'s `headers` array. Rules
compile in minimatch-only mode (no path-to-regexp routing), accumulate
in declaration order with case-insensitive last-write-wins semantics,
layer over default response headers on success and on non-JSON 4xx
responses, and skip 3xx redirects entirely. `value: null` entries prune
a header whose final accumulated state is null, mirroring
`serve-handler`'s two-stage `Object.assign`-then-prune merge.
