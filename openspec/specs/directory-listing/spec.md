...
Behavior when a request resolves to a directory without an
`index.html`: HTML and JSON listings (with content negotiation), the
`unlisted` filter and default-excluded set, and `renderSingle` for
single-file directories. JSON-listing path-leak sanitization (D-007) is
honored here.
