...
URL transformations applied before file resolution: `cleanUrls`
(extensionless `.html` resolution and 301), `trailingSlash` add/strip,
multi-slash path normalization, and the operation precedence between
routing stages (cleanUrls → trailingSlash → redirects → rewrites →
cleanUrls-resolution → static file). This capability owns the request
pipeline up to the static-file resolution boundary.
