...
Command-line interface for the IrServe static-file server: argument parsing,
exit codes, and the runtime knobs exposed to operators. Behaviors here are
the observable surface the CLI presents to users invoking the binary
directly; per-request HTTP behavior driven by these flags lives in the
